{
  ...
  programs.nixvim = {
    ...
    plugins = {
      ...
      lsp = {
        ...
        servers = {
          ...
          # jdtls.enable = true; # Handled by jdtls plugin
          ts_ls.enable = true;
          bashls.enable = true;
          gopls.enable = true;
          jsonls.enable = true;
    ...
    keymaps = [
      ...
      # Commenting (using remap to trigger comment.nvim)
      {
        mode = "n";
        key = "<leader>/";
        action = "gcc";
    ...
    autoCmd = [
      ...
      # Auto Save logic
      {
        ...
        callback.__raw = ''
          function()
            if _G.save_timer then
              vim.uv.timer_stop(_G.save_timer)
            end
            _G.save_timer = vim.uv.new_timer()
            vim.uv.timer_start(
              _G.save_timer,
              115,
              0,
